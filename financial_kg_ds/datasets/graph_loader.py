...
class GraphLoaderRegresion(GraphLoaderBase):
    ...
    def add_ticker_node(self):
        ticker_x, ticker_mapping = self.load_node_csv(self.data_path+'/ticker_info.csv', 'ticker', encoders={'ticker': OneHotEncoder(),
    ...
    def add_mutual_fund_node(self):
        mutual_fund_x, mutual_fund_mapping = self.load_node_csv(self.data_path+'/mutual_fund.csv', 'name', encoders={'name': OneHotEncoder()}, na_values={'value': 0})
        self.data['mutual_fund'].x = mutual_fund_x.float()
        self.mutual_fund_mapping = mutual_fund_mapping
    
    def add_institution_node(self):
        institution_x, institution_mapping = self.load_node_csv(self.data_path+'/institution.csv', 'name', encoders={'name': OneHotEncoder()}, na_values={'value': 0})
        self.data['institution'].x = institution_x.float()
        self.institution_mapping = institution_mapping

    def add_news_node(self):
        print('loading news')
        news_x, news_mapping = self.load_node_csv(self.data_path+'/news.csv', 'title', encoders={'title': SentimentAnalysisEncoder()})
        self.data['news'].x = news_x.float()
        self.news_mapping = news_mapping

    def add_holds_it_rel(self):
        edge_index, edge_attr = self.load_edge_csv(self.data_path+'/institution.csv', 'ticker', self.ticker_mapping, 'name', self.institution_mapping, encoders={'value': IdentityEncoder(), 'pctHeld': IdentityEncoder()})
    ...
    def add_holds_mt_rel(self):
        edge_index, edge_attr = self.load_edge_csv(self.data_path+'/mutual_fund.csv', 'ticker', self.ticker_mapping, 'name', self.mutual_fund_mapping, encoders={'value': IdentityEncoder(), 'pctHeld': IdentityEncoder()})
    ...
    def about_nt_rel(self):
        edge_index, edge_attr = self.load_edge_csv(self.data_path+'/news.csv', 'title', self.news_mapping, 'ticker', self.ticker_mapping)
...
class GraphLoaderAE(GraphLoaderBase):
    ...
    # Upadate hetero data with node and edge data
    def add_ticker_node(self):
        ticker_x, ticker_mapping = self.load_node_csv(self.data_path+'/ticker_info.csv', 'ticker', encoders={'ticker': OneHotEncoder(),'currentPrice': IdentityEncoder()}) # ,'targetMeanPrice': IdentityEncoder(), 'marketCap': IdentityEncoder()})
        self.data['ticker'].x = ticker_x.float()
        self.ticker_mapping = ticker_mapping

    def add_mutual_fund_node(self):
        mutual_fund_x, mutual_fund_mapping = self.load_node_csv(self.data_path+'/mutual_fund.csv', 'name', encoders={'name': OneHotEncoder()})
        self.data['mutual_fund'].x = mutual_fund_x.float()
        self.mutual_fund_mapping = mutual_fund_mapping
    
    def add_institution_node(self):
        institution_x, institution_mapping = self.load_node_csv(self.data_path+'/institution.csv', 'name', encoders={'name': OneHotEncoder()})
        self.data['institution'].x = institution_x.float()
        self.institution_mapping = institution_mapping
    ...
    def add_holds_it_rel(self):
        edge_index, edge_attr = self.load_edge_csv(self.data_path+'/institution.csv', 'ticker', self.ticker_mapping, 'name', self.institution_mapping, encoders={'value': IdentityEncoder()})
        self.data['ticker', 'holds_it', 'institution'].edge_index = edge_index
        self.data['ticker', 'holds_it', 'institution'].edge_attr = edge_attr

    def add_holds_mt_rel(self):
        edge_index, edge_attr = self.load_edge_csv(self.data_path+'/mutual_fund.csv', 'ticker', self.ticker_mapping, 'name', self.mutual_fund_mapping, encoders={'value': IdentityEncoder()})
        self.data['ticker', 'holds_mt', 'mutual_fund'].edge_index = edge_index
        self.data['ticker', 'holds_mt', 'mutual_fund'].edge_attr = edge_attr

    def about_nt_rel(self):
        edge_index, edge_attr = self.load_edge_csv(self.data_path+'/news.csv', 'uuid', self.news_mapping, 'ticker', self.ticker_mapping)
        self.data['news', 'about_nt', 'ticker'].edge_index = edge_index
        self.data['news', 'about_nt', 'ticker'].edge_attr = edge_attr
